...
RobotControlServiceImpl::RobotControlServiceImpl(
  std::shared_ptr<rclcpp::Node> node) : node_(node), servo_joint_msg_(), servo_tcp_msg_() {
    servo_joint_pub_ =
        node_->create_publisher<control_msgs::msg::JointJog>("/servo_node/delta_joint_cmds", 10);
    servo_tcp_pub_ =
        node_->create_publisher<geometry_msgs::msg::TwistStamped>("/servo_node/delta_twist_cmds", 10);
    ...
    pub_timer_ = node_->create_wall_timer(pub_period, std::bind(&RobotControlServiceImpl::PublishLoop, this));
}
...
grpc::Status RobotControlServiceImpl::ServoTCP(
    grpc::ServerContext* context,
...
void RobotControlServiceImpl::GripperSendGoal(double position, double max_effort) {
    if (!gripper_client_->wait_for_action_server()) {
      RCLCPP_ERROR(node_->get_logger(), "Gripper Controller action server is not available. Skipping goal.");
      return;
    }
    ...
    options.result_callback =
      [this](const GoalHandleGripper::WrappedResult & result)
      {
        ...
        RCLCPP_ERROR(this->node_->get_logger(), "Gripper goal result: %s", gripper_cmd_msg_.c_str());
      };
